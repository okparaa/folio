...
describe("Roles Controller", () => {
  ...
  describe("addRolePermissions", () => {
    ...
    it("should return 400 when validation fails", async () => {
      ...
      await rolesController.addRolePermissions(req as Request, res as Response);
    ...
    it("should return 404 when service return undefined", async () => {
      validationResult.output = role;
      validationResult.issues = null;
      ...
      // Mock successful validation but no role found
      const spyService = jest
        .spyOn(rolesController.service, "addRolePermissions")
        .mockResolvedValue(undefined);
      ...
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({
        message: "Added permissions not returned",
      });
    });

    it("should return 200 when permissions is added to role", async () => {
      validationResult.output = role;
      validationResult.issues = null;
      ...
      expect(spyService).toHaveBeenCalledTimes(1);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(role.permissions);
    });
  ...
  describe("delRolePermisions", () => {
    const validation = validationResult as SafeParseResult<
      typeof RolesPermitsSchema
    >;
    it("should return 400 when validation fail", async () => {
      ...
      await rolesController.delRolePermisions(req as Request, res as Response);
    ...
    it("should return 404 when service return undefined", async () => {
      ...
      const spyService = jest
        .spyOn(rolesController.service, "delRolePermissions")
        .mockResolvedValue(undefined);
      ...
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({
        message: "Deleted permissions not returned",
      });
    });
    ...
    describe("deleteRole", () => {
      it("should always return 403 forbidden", async () => {
        await rolesController.deleteRole(req as Request, res as Response);

        expect(res.status).toHaveBeenCalledWith(403);
        expect(res.json).toHaveBeenCalledWith({
          message: "Role deletion is forbidden",
        });
      });
    });
  });
